...
const ManageCollection = ({ handleClose, updateDestination, date, fetchTrips }: ManageCollectionProps) => {
  ...
  useEffect(()=> {
    (async () => {
      const  desList = await axios.get(`http://localhost:3000/api/v1/trips/${trip._id}/destinations`, { headers: {Authorization: `Bearer ${user}`}});
 ...
 const handleSubmitUpdate = async (e) => {
  ...
  const UpdatedTrip = await axios.patch(
    `http://localhost:3000/api/v1/trips/${trip._id}`,
    ...
    if (updatedDesList !== destinations){
      try {
        for ( const destination of updatedDesList) {
          const updateDesInfo = await axios.patch(`http://localhost:3000/api/v1/trips/${trip._id}/destinations/${destination._id}`, {name: destination.name}, { headers: { Authorization: `Bearer ${user}`}});
        }
      } catch (error) {
    ...
    if (destIdToDelete){
      try {
        for (const destinationId of destIdToDelete) {
          const updateDesList = await axios.delete(`http://localhost:3000/api/v1/trips/${trip._id}/destinations/${destinationId}`, {headers: {Authorization: `Bearer ${user}`}});
        }
      } catch (error) {
