...
const DestinationActivity = ({ destination }:{ destination: Destination } ) => {
  ...
  const handleActDel = (currentActivity: string) => {
    ...
    ( async () => {
      await axios.patch(`http://localhost:3000/api/v1/trips/${trip._id}/destinations/${destination._id}`,{activities: updatedActivity}, { headers: { Authorization: `Bearer ${user}`}});
    })();
  }
  ...
  useEffect(()=> {
    if(activityList.length){
      (async () => {
        await axios.patch(`http://localhost:3000/api/v1/trips/${trip._id}/destinations/${destination._id}`,{activities: activityList}, { headers:{ Authorization: `Bearer ${user}`}});
      })();
    }
  }, [activityList]);
