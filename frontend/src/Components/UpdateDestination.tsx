...
const UpdateDestination = ({ handleClose, desId }: UpdateDesProps) => {
  ...
  useEffect(() => {
    (async () => {
      const data = await axios.get(`http://localhost:3000/api/v1/trips/${trip._id}/destinations/${desId}`, { headers: { Authorization: `Bearer ${user}` } });
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const newDestination = await axios.patch(
      `http://localhost:3000/api/v1/trips/${trip._id}/destinations/${desId}`,
